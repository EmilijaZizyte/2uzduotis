...
Rezultatai strategija2_STL_vectoriui(const std::string& failas) {
    ...
    while (std::getline(open_f, eilute)) {
        ...
        Studentas s;
        ss >> s.vard >> s.pav;
        s.paz.resize(5);  // rezervuojame vietą 5 pažymiams
    ...
    // ================= Atmintis =================
    size_t totalBytes = 0;
    for (auto& s : visiStudentaiV) totalBytes += sizeof(Studentas) + s.paz.capacity() * sizeof(int);
    for (auto& s : vargsiukaiV) totalBytes += sizeof(Studentas) + s.paz.capacity() * sizeof(int);
    rez.memMB = totalBytes / (1024.0 * 1024.0);
